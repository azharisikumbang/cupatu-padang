            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-center border">Total Pesanan Diselesaikan</th>
                        <th scope="col" class="px-6 py-3 text-center border">Total Pesanan Dibatalkan</th>
                    </tr>
                </thead>
                <tbody>
                    @php
                    ...
                    @forelse($data as $report)
                        @php
                            $totalSelesai += $report->jumlah_pesanan_selesai;
                            $totalBatal += $report->jumlah_pesanan_batal;
                        @endphp
                        <tr class="border-b">
                            <td class="text-center px-6 py-3 border">{{ $loop->iteration }}</td>
                            <td class="text-center px-6 py-3 border">{{ $listBulan[$report->bulan - 1] }}</td>
                            <td class="text-center px-6 py-3 border">{{ $report->jumlah_pesanan_selesai }}</td>
                            <td class="text-center px-6 py-3 border">{{ $report->jumlah_pesanan_batal }}</td>
                        </tr>
                    @empty
                        <tr>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="text-center px-6 py-3 border">{{ $totalSelesai }}</td>
                        <td class="text-center px-6 py-3 border">{{ $totalBatal }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
